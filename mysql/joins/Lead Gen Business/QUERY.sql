...
-- JOINIG TABLES
-- FIND ALL THE CLIENTS(FIRST NAME LAST NAME)BILLINHG AMMOUNTS AND CHARGED DATE
...
-- get the names of the cliets, their domain names and the first name of all the leads  generated from those site.
SELECT clients.first_name AS client_first, clients.last_name, sites.domain_name, leads.first_name AS leads_first 
FROM clients
JOIN sites ON clients.id = sites.clients_id
JOIN leads ON sites.id  = leads.sites_id;
-- LEFT AND RIGHT JOIN 
-- LIST ALL TRE CLIENTS AND SITES EACH CLIENT HAS, EVEN THE CLIENT HASNT LANDED A SITE YET
...
-- --GROUP BY
-- --SUM,MIN,MAX,AVG
-- FIND ALL THE CLIENTS FULL NAME AND THEIR TOTAL BILLING AMOUNT
SELECT clients.first_name, clients.last_name, SUM(billing.amount)
FROM clients
JOIN billing ON clients.id = billing.clients_id
GROUP BY clients.id;
-- GROUP CONTACT
-- LIST ALL THE DOMAIN NAMES ASSOSIATED WITH EACH CLIENT 
SELECT GROUP_CONCAT(' ',sites.domain_name) AS domains, clients.first_name, clients.last_name
FROM clients
JOIN sites ON clients.id = sites.clients_id
GROUP BY clients.id;
-- COUNT() 
-- find the total number of leads for each site.  count(*) as num_transactions,
...
-- to see total spending of the client AND CHANGING CODE AS NAMING PREFERED- 
SELECT sum(amount) as total, clients.id, clients.first_name, clients.last_name, clients.email FROM billing JOIN clients ON billing.clients_id = clients.id GROUP BY clients.id;
-- to see how many tim each client payed
SELECT sum(amount) as total, count(*) as num_transaction, clients.id, clients.first_name as first, clients.last_name as last, clients.email 
FROM billing 
JOIN clients ON billing.clients_id  = clients.id GROUP BY clients.id, YEAR(charged_datetime);
--  SELECT sum(amount) as total, clients.id, clients.frist_name, clients.last_name as last, clients.email FROM billing as b1 JOIN clients as c1 ON billing.clients_id  = clients.id GROUP BY clients.id, YEAR(charged_datetime);